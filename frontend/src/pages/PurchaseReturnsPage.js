...
// import { Plus, RotateCcw, Printer, X } from "lucide-react";
...
//   const [editForm,    setEditForm]    = useState({ date: '', reason: '', notes: '' });
...
//       const { data } = await purchaseReturnsAPI.get(id);
//       setEditReturn(data);
//       setEditForm({ date: data.date, reason: data.reason, notes: data.notes || '' });
...
//     try {
//       await purchaseReturnsAPI.update(editReturn.id, { ...editForm, items: returnItems });
//       toast.success("Purchase return updated");
//       setEditModal(false);
//       load();
//       // Refresh detail panel if this return is open
//       if (selected?.id === editReturn.id) {
//         const { data } = await purchaseReturnsAPI.get(editReturn.id);
//         setSelected(data);
//       }
...
//     doc.text('Purchase Return Note', pageW / 2, 16, { align: 'center' });
...
//     doc.text(ret.supplier_name || '-', pageW / 2, y + 12, { align: 'center' });
//     doc.setTextColor(148, 163, 184); doc.setFont('helvetica', 'normal'); doc.setFontSize(7);
...
//     doc.text('Against Purchase:', 13, y + 6);
//     doc.setFont('helvetica', 'bold'); doc.setTextColor(251, 191, 36);
//     doc.text(ret.purchase_number || '-', 45, y + 6);
//     doc.setFont('helvetica', 'normal'); doc.setTextColor(148, 163, 184);
//     doc.text('Reason:', pageW / 2, y + 6);
//     doc.setFont('helvetica', 'italic'); doc.setTextColor(200, 210, 220);
//     doc.text(ret.reason || '-', pageW / 2 + 16, y + 6);
//     y += 15;
...
//     doc.text('TOTAL RETURN AMOUNT', lX, tY);
//     doc.setTextColor(251, 191, 36);
...
//     doc.text('GH & Sons Enterprises — Purchase Return Receipt', pageW / 2, doc.internal.pageSize.getHeight() - 6, { align: 'center' });

//     doc.save(`purchase-return-${ret.return_number}.pdf`);
//   };
...
//                 </span>
//               </div>
//               <button onClick={() => setReceiptReturn(null)} className="text-slate-500 hover:text-slate-300 transition-colors">
//                 <X size={16} />
//               </button>
//             </div>
...
//                 <p className="text-slate-500 text-xs mt-0.5">Purchase Return Note</p>
//               </div>
...
//                   { label: 'Supplier',  value: receiptReturn.supplier_name,  cls: 'text-slate-200 font-semibold truncate' },
...
//                   <p className="text-[9px] text-slate-500 uppercase tracking-wider mb-0.5" style={{ fontFamily: 'Barlow Condensed' }}>Against Purchase</p>
//                   <p className="text-xs text-amber-400 font-mono">{receiptReturn.purchase_number}</p>
//                 </div>
//                 <div className="bg-[#0A0F1C] rounded-sm px-3 py-2 border border-[#1E2738]">
...
//                 <span className="text-slate-500 text-xs uppercase tracking-wider" style={{ fontFamily: 'Barlow Condensed' }}>Total Return Amount</span>
//                 <span className="text-amber-400 font-mono font-bold">PKR {receiptReturn.total_amount?.toLocaleString()}</span>
//               </div>

//               {receiptReturn.notes && (
//                 <p className="text-xs text-slate-500 italic px-1">Note: {receiptReturn.notes}</p>
//               )}
//             </div>
...
//               </button>
//               <button onClick={() => setReceiptReturn(null)}
...
//               </h3>
//               <button type="button" onClick={() => setEditModal(false)} className="text-slate-500 hover:text-white transition-colors">
//                 <X size={18} />
//               </button>
//             </div>
...
//               {/* Items — lot locked, only qty editable */}
...
//                         {((parseFloat(item.quantity)||0) * (item.rate||0)).toLocaleString()}
//                       </p>
//                     </div>
//                   </div>
//                 ))}
...
export default function PurchaseReturnsPage() {
  ...
  const loadReturns   = () => purchaseReturnsAPI.list().then(r => setReturns(r.data));
  const loadPurchases = () => purchasesAPI.list().then(r => setPurchases(r.data));
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setItems([]);
      setPurchaseDetail(null);
      // Reload both — a full return removes the purchase from purchases list
      loadReturns();
      loadPurchases();
    } catch (err) {
      toast.error(err.response?.data?.error || "Error creating return");
              ...
              {selected.items?.length > 0 && (
                <div className="mt-4">
                  ...
                  {selected.items.map((item, i) => (
                    <div key={item.id || i} className="bg-[#0A0F1C] border border-[#2D3648] p-3 rounded-sm mb-2 text-xs space-y-1">
                      <div className="flex justify-between">
                        <span className="text-slate-300 font-medium">{item.product_name || '—'}</span>
                        <span className="text-amber-400 font-mono">{item.lot_number || '—'}</span>
                      </div>
